...
{
	uint8_t i;
	digitalWrite(OLED_RS_PIN, cmd); //
	digitalWrite(OLED_CS_PIN, LOW);
	for(i=0;i<8;i++)
	{
		digitalWrite(OLED_SCLK_PIN, LOW);
		if(dat&0x80) {
            digitalWrite(OLED_SDIN_PIN, HIGH);
        }
		else {
            digitalWrite(OLED_SDIN_PIN, LOW);
        }
		digitalWrite(OLED_SCLK_PIN, HIGH);
		dat<<=1;
	}
